...
GENERATED_VIEW_HEADER = "<!-- Generated by `uv run fo view build`; do not edit by hand. -->"
VIEW_TARGET_KINDS = {"relationship", "thesis"}
CHAIN_IMPACT_SEED_KINDS = VIEW_TARGET_KINDS | {
    "challenge",
    "claim",
    "dataset",
    "event",
...
QUESTION_CHAIN_REFERENCE_FIELDS = (
    "related_evidence",
    "related_claims",
...
def artifact_markdown_link(label: str, from_path: Path, target_path: Path) -> str:
    relative = os.path.relpath(target_path, start=from_path.parent)
    return f"[{label}]({relative})"


def repo_markdown_link(root: Path, label: str, target_path: Path) -> str:
    return f"[{label}]({relative_path(root, target_path)})"


def record_markdown_link(root: Path, record_id: str, id_map: dict[str, Record]) -> str:
    record = id_map.get(record_id)
    if not record:
        return f"`{record_id}`"
    return repo_markdown_link(root, f"`{record_id}`", record.path)


def record_link_list(
    root: Path,
    record_ids: list[str],
    id_map: dict[str, Record],
    limit: int = 20,
    ...
    if not record_ids:
        return "None"
    links = [record_markdown_link(root, record_id, id_map) for record_id in record_ids[:limit]]
    if len(record_ids) > limit:
        links.append(f"... {len(record_ids) - limit} more")
    return ", ".join(links)


def view_records_for_target(
    root: Path, records: list[Record], target_id: str, kind: str
) -> list[dict[str, Any]]:
    return [
        record_doc(root, record, include_data=True)
        for record in sorted(records, key=lambda item: item.id)
        if record.kind == kind and record.data.get("target") == target_id
    ]


def analysis_for_view_target(
    root: Path,
    records: list[Record],
    target: Record,
    id_map: dict[str, Record],
    ...
    analysis = build_review_analysis(
        target,
        view_records_for_target(root, records, target.id, "validation"),
        view_records_for_target(root, records, target.id, "challenge"),
        id_map,
        target_archived=False,
    )
    analysis["chain_summary"] = build_chain_summary(target, analysis, id_map)
    return analysis


def generated_view_slug(record_id: str) -> str:
    return id_slug(record_id).replace("_", "-")


def github_view_page_path(record: Record) -> Path:
    return Path("chains") / record.kind / f"{generated_view_slug(record.id)}.md"


def render_count_map(value: dict[str, Any]) -> str:
    if not value:
        return "None"
    return ", ".join(f"`{key}`={value[key]}" for key in sorted(value))


def render_stage_coverage(stage_coverage: dict[str, Any]) -> str:
    covered = [key for key, enabled in stage_coverage.items() if enabled]
    missing = [key for key, enabled in stage_coverage.items() if not enabled]
    return (
        f"Covered: {', '.join(f'`{key}`' for key in covered) or 'none'}\n\n"
        f"Missing: {', '.join(f'`{key}`' for key in missing) or 'none'}"
    )


def chain_expansion_seed_ids_for_record(record: Record) -> set[str]:
    seed_ids: set[str] = set()
    if record.kind in {"challenge", "validation"}:
        target = record.data.get("target")
        if isinstance(target, str):
            seed_ids.add(target)
    if record.kind == "question":
        for field_name in QUESTION_CHAIN_REFERENCE_FIELDS:
            seed_ids.update(as_reference_list(record.data.get(field_name)))
    return seed_ids


def changed_chain_seed_map(
    delta: dict[str, Any],
    base_records: list[Record] | None = None,
    current_records: list[Record] | None = None,
) -> dict[str, set[str]]:
    before = record_map(base_records or [])
    after = record_map(current_records or [])
    ...
    for section in ("added", "modified", "deleted", "renamed"):
        ...
        for item in items:
            ...
            kind = item.get("kind")
            if isinstance(record_id, str) and kind in CHAIN_IMPACT_SEED_KINDS:
                seed_ids = seeds.setdefault(record_id, set())
                seed_ids.add(record_id)
                for record in (before.get(record_id), after.get(record_id)):
                    if record:
                        seed_ids.update(chain_expansion_seed_ids_for_record(record))
    return seeds


def changed_chain_seed_ids(
    delta: dict[str, Any],
    base_records: list[Record] | None = None,
    current_records: list[Record] | None = None,
) -> set[str]:
    seed_map = changed_chain_seed_map(delta, base_records, current_records)
    return {seed_id for seed_ids in seed_map.values() for seed_id in seed_ids}


def incoming_reference_index(records: list[Record]) -> dict[str, set[str]]:
    incoming: dict[str, set[str]] = {}
    for record in records:
        if not record.id:
            continue
        for _, target_id, _ in record_ref_rows(record):
            incoming.setdefault(target_id, set()).add(record.id)
    return incoming


def impacted_chain_target_map(
    root: Path, records: list[Record], seed_map: dict[str, set[str]]
) -> dict[str, list[str]]:
    by_id = record_map(records)
    incoming = incoming_reference_index(records)
    target_seeds: dict[str, set[str]] = {}
    queue: deque[tuple[str, str]] = deque(
        (origin_id, seed_id)
        for origin_id in sorted(seed_map)
        for seed_id in sorted(seed_map[origin_id])
    )
    visited: set[tuple[str, str]] = set()

    while queue:
        origin_id, record_id = queue.popleft()
        key = (origin_id, record_id)
        if key in visited:
            continue
        visited.add(key)

        record = by_id.get(record_id)
        if record and record.kind in VIEW_TARGET_KINDS and not is_archived_path(root, record.path):
            target_seeds.setdefault(record_id, set()).add(origin_id)

        if record and (
            record.kind in {"challenge", "validation"}
            or (record.kind == "question" and origin_id == record.id)
        ):
            for seed_id in chain_expansion_seed_ids_for_record(record):
                next_key = (origin_id, seed_id)
                if next_key not in visited:
                    queue.append((origin_id, seed_id))

        for referrer_id in sorted(incoming.get(record_id, set())):
            next_key = (origin_id, referrer_id)
            if next_key not in visited:
                queue.append((origin_id, referrer_id))

    return {target_id: sorted(seed_ids) for target_id, seed_ids in sorted(target_seeds.items())}


def impacted_chain_target_ids(root: Path, records: list[Record], seed_ids: set[str]) -> list[str]:
    seed_map = {seed_id: {seed_id} for seed_id in sorted(seed_ids)}
    return sorted(impacted_chain_target_map(root, records, seed_map))


def chain_impact_summary(root: Path, records: list[Record], seed_map: dict[str, set[str]]) -> dict[str, Any]:
    target_map = impacted_chain_target_map(root, records, seed_map)
    expanded_seed_ids = sorted({seed_id for seed_ids in seed_map.values() for seed_id in seed_ids})
    return {
        "seed_map": {origin_id: sorted(seed_ids) for origin_id, seed_ids in sorted(seed_map.items())},
        "seed_ids": sorted(seed_map),
        "expanded_seed_ids": expanded_seed_ids,
        "target_ids": sorted(target_map),
        "target_count": len(target_map),
        "by_target": [
            {"id": target_id, "seed_ids": target_map[target_id]}
            for target_id in sorted(target_map)
        ],
    }


def chain_seed_map_from_payload(diff_payload: dict[str, Any]) -> dict[str, set[str]]:
    chain_impact = diff_payload.get("chain_impact", {})
    if isinstance(chain_impact, dict):
        seed_map = chain_impact.get("seed_map")
        if isinstance(seed_map, dict):
            parsed: dict[str, set[str]] = {}
            for origin_id, seed_ids in seed_map.items():
                if isinstance(origin_id, str) and isinstance(seed_ids, list):
                    parsed[origin_id] = {str(seed_id) for seed_id in seed_ids}
            if parsed:
                return parsed
        seed_ids = chain_impact.get("seed_ids")
        if isinstance(seed_ids, list):
            return {str(seed_id): {str(seed_id)} for seed_id in seed_ids}
    return {
        seed_id: {seed_id}
        for seed_id in changed_chain_seed_ids(diff_payload.get("record_delta", {}))
    }


def changed_view_targets(root: Path, records: list[Record], diff_payload: dict[str, Any]) -> list[Record]:
    by_id = record_map(records)
    target_map = impacted_chain_target_map(root, records, chain_seed_map_from_payload(diff_payload))
    return [by_id[record_id] for record_id in sorted(target_map) if record_id in by_id]


def render_chain_view_page(
    root: Path,
    target: Record,
    analysis: dict[str, Any],
    id_map: dict[str, Record],
) -> tuple[Path, str]:
    page_relative_path = github_view_page_path(target)
    chain = analysis["chain_summary"]
    review_state = analysis["review_state"]
    source_chain = chain["source_evidence_chain"]
    relationship_chain = chain["relationship_chain"]
    question_summary = chain["question_summary"]
    challenge_summary = chain["challenge_summary"]
    dependencies = chain["dependency_ids"]
    pressure = chain["relationship_promotion_pressure"]

    lines = [
        GENERATED_VIEW_HEADER,
        "",
        f"# {record_label(target)}",
        "",
        "Derived review state is deterministic local output, not canonical truth.",
        "",
        f"- ID: `{target.id}`",
        f"- Kind: `{target.kind}`",
        f"- Canonical record: {repo_markdown_link(root, relative_path(root, target.path), target.path)}",
        f"- Review state: `{review_state['primary_label']}`",
        f"- Review flags: {', '.join(f'`{flag}`' for flag in review_state['flags']) or 'None'}",
        "",
        "## Chain Snapshot",
        "",
        f"- Evidence: {source_chain['evidence_count']} records across {source_chain['source_count']} sources.",
        f"- Evidence classes: {render_count_map(source_chain['evidence_class_counts'])}",
        f"- Source perspectives: {render_count_map(source_chain['source_perspective_counts'])}",
        f"- Dependency counts: {render_count_map(chain['dependency_counts'])}",
        f"- Relationship types: {render_count_map(relationship_chain['type_counts'])}",
        f"- Strong relationship IDs: {record_link_list(root, pressure['strong_relationship_type_ids'], id_map)}",
        f"- Promotion pressure flags: {', '.join(f'`{flag}`' for flag in pressure['promotion_risk_flags']) or 'None'}",
        f"- Open questions: {question_summary['open_count']}",
        f"- Open challenges: {challenge_summary['open_count']}",
        "",
        "## Stage Coverage",
        "",
        render_stage_coverage(chain["stage_coverage"]),
        "",
        "## Dependency Records",
        "",
        f"- Claims: {record_link_list(root, dependencies['claims'], id_map)}",
        f"- Relationships: {record_link_list(root, dependencies['relationships'], id_map)}",
        f"- Metrics: {record_link_list(root, dependencies['metrics'], id_map)}",
        f"- Events: {record_link_list(root, dependencies['events'], id_map)}",
        f"- Datasets: {record_link_list(root, dependencies['datasets'], id_map)}",
        "",
        "## Source Evidence",
        "",
    ]

    if source_chain["items"]:
        lines.extend(
            [
                "| Evidence | Class | Source | Perspective | Observed |",
                "| --- | --- | --- | --- | --- |",
            ]
        )
        for item in source_chain["items"]:
            ...
            lines.append(
                ...
                + " | ".join(
                    [
                        record_markdown_link(root, evidence_id, id_map),
                        markdown_cell(item["evidence_class"]),
                        record_markdown_link(root, source_id, id_map),
                        markdown_cell(item["source_perspective"]),
                        markdown_cell(item["observed_at"]),
                    ]
                )
                + " |"
            )
    else:
        lines.append("No source evidence surfaced in the chain summary.")
    ...
    lines.extend(
        [
            ...
            "",
            f"- Open question IDs: {record_link_list(root, question_summary['open_question_ids'], id_map)}",
            f"- Open challenge IDs: {record_link_list(root, challenge_summary['open_challenge_ids'], id_map)}",
            "- Risk flags: "
            + (", ".join(f"`{flag}`" for flag in chain["risk_flag_summary"]["all_risk_flags"]) or "None"),
            "",
        ]
    )
...
def warning_code_counts(warnings: list[dict[str, Any]]) -> dict[str, int]:
    return sorted_counts([str(warning.get("code")) for warning in warnings])


def render_diff_review_summary(diff_payload: dict[str, Any]) -> list[str]:
    total = diff_payload.get("record_delta", {}).get("total", {})
    graph = diff_payload.get("graph_impact", {})
    ontology = diff_payload.get("ontology_impact", {})
    chain_impact = diff_payload.get("chain_impact", {})
    warnings = diff_payload.get("warnings", [])
    return [
        "## Diff Review",
        "",
        f"- Record delta: +{total.get('added', 0)} / ~{total.get('modified', 0)} / -{total.get('deleted', 0)} / renamed {total.get('renamed', 0)}.",
        f"- Graph edge delta: +{graph.get('added_edge_count', 0)} / -{graph.get('removed_edge_count', 0)}.",
        f"- Changed or impacted chains: {chain_impact.get('target_count', 0)}.",
        f"- Ontology changes: {ontology.get('changed_count', 0)}.",
        f"- Warning codes: {render_count_map(warning_code_counts(warnings))}",
        "",
    ]


def render_pr_review_markdown(
    root: Path,
    output_dir: Path,
    base_ref: str,
    ...
    lines = [
        ...
        *render_diff_review_summary(diff_payload),
        "## Changed Or Impacted Thesis/Relationship Chains",
        "",
    ]
    if chain_pages:
        lines.extend(
            [
                "| Target | Impacted By | Kind | Review State | Evidence | Sources | Open Questions | Open Challenges |",
                "| --- | --- | --- | --- | ---: | ---: | ---: | ---: |",
            ]
        )
        pr_path = output_dir / "pr-review.md"
        for item in chain_pages:
            page_path = output_dir / Path(str(item["page"]))
            lines.append(
                "| "
                + " | ".join(
                    [
                        artifact_markdown_link(markdown_cell(item["label"]), pr_path, page_path),
                        ", ".join(f"`{seed_id}`" for seed_id in item.get("seed_ids", [])) or "direct",
                        f"`{item['kind']}`",
                        f"`{item['review_state']['primary_label']}`",
                        str(item["evidence_count"]),
                        str(item["source_count"]),
                        str(item["open_question_count"]),
                        str(item["open_challenge_count"]),
                    ]
                )
                + " |"
            )
    else:
        lines.append("No changed or impacted current thesis or relationship records were found.")
    lines.append("")
    return "\n".join(lines)


def render_github_view(
    root: Path, output_dir: Path, base_ref: str
) -> tuple[dict[Path, str], dict[str, Any]]:
    diff_payload, diff_status = build_diff_review(root, base_ref)
    if diff_status:
        messages = [error["message"] for error in diff_payload.get("errors", [])] or [
            f"diff-review failed for `{base_ref}`"
        ]
        raise ValueError("\n".join(messages))
    ...
    target_map = impacted_chain_target_map(
        root,
        records,
        chain_seed_map_from_payload(diff_payload),
    )
    targets = [id_map[record_id] for record_id in sorted(target_map) if record_id in id_map]
    rendered: dict[Path, str] = {}
    chain_pages: list[dict[str, Any]] = []

    for target in targets:
        analysis = analysis_for_view_target(root, records, target, id_map)
        relative_path, content = render_chain_view_page(root, target, analysis, id_map)
        ...
        chain_pages.append(
            {
                ...
                "kind": target.kind,
                "label": record_label(target),
                "page": str(relative_path),
                "seed_ids": target_map.get(target.id, []),
                "review_state": analysis["review_state"],
                "evidence_count": analysis["chain_summary"]["source_evidence_chain"][
                    "evidence_count"
                ],
                "source_count": analysis["chain_summary"]["source_evidence_chain"]["source_count"],
                "open_question_count": analysis["chain_summary"]["question_summary"]["open_count"],
                "open_challenge_count": analysis["chain_summary"]["challenge_summary"]["open_count"],
            }
        )

    rendered[Path("pr-review.md")] = render_pr_review_markdown(
        root, output_dir, base_ref, diff_payload, chain_pages
    )
    ...
    lines = [
        GENERATED_VIEW_HEADER,
        "",
        "# Finance OSINT GitHub View",
        "",
        "This directory is generated under `.local/` for GitHub PR review.",
        "Canonical data remains under `records/` and `ontology/`.",
        "",
        f"- PR summary: {artifact_markdown_link('pr-review.md', index_path, output_dir / 'pr-review.md')}",
        f"- Chain pages: {len(chain_pages)}",
        "",
    ]
    rendered[Path("index.md")] = "\n".join(lines)

    summary = {
        "base": base_ref,
        "base_sha": diff_payload.get("base_sha"),
        "changed_or_impacted_target_count": len(targets),
...
def existing_generated_markdown(output_dir: Path) -> dict[Path, str]:
    if not output_dir.exists():
        return {}
    files: dict[Path, str] = {}
    for path in sorted(output_dir.rglob("*.md")):
        files[path.relative_to(output_dir)] = path.read_text(encoding="utf-8")
    return files


def write_github_view(output_dir: Path, rendered: dict[Path, str]) -> None:
...
def view_diff(existing: dict[Path, str], rendered: dict[Path, str]) -> dict[str, list[str]]:
    existing_paths = set(existing)
    rendered_paths = set(rendered)
    return {
        "missing": [str(path) for path in sorted(rendered_paths - existing_paths)],
        "extra": [str(path) for path in sorted(existing_paths - rendered_paths)],
        "changed": [
            str(path)
            for path in sorted(existing_paths & rendered_paths)
            if existing[path].rstrip() != rendered[path].rstrip()
        ],
    }
...
def run_view_build(
    ...
    try:
        rendered, summary = render_github_view(root, output_dir, base_ref)
    except ValueError as exc:
        errors = [json_error(error) for error in str(exc).splitlines()]
        if json_output:
            print_json(result_envelope("view build", root, ok=False, errors=errors))
        else:
            for error in errors:
    ...
    if check:
        diff = view_diff(existing_generated_markdown(output_dir), rendered)
        ...
        if json_output:
            ...
            print("Generated GitHub view is current" if ok else "Generated GitHub view is stale")
        return 0 if ok else 1

    write_github_view(output_dir, rendered)
    payload = result_envelope("view build", root, output_dir=str(output_dir), **summary)
    if json_output:
        print_json(payload)
    else:
        print(f"Wrote {summary['page_count']} GitHub view page(s) to {output_dir}")
    return 0
...
def build_diff_review(root: Path, base_ref: str) -> tuple[dict[str, Any], int]:
    ...
    chain_seeds = changed_chain_seed_map(delta, base_records, current_records)
    current_chain_records = [
        record for record in current_records if not is_archived_path(root, record.path)
    ]
    chain_impact = chain_impact_summary(root, current_chain_records, chain_seeds)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    view_parser = subparsers.add_parser("view", help="GitHub-readable generated view utilities")
    view_subparsers = view_parser.add_subparsers(dest="view_command", required=True)

    view_build_parser = view_subparsers.add_parser(
        "build",
    ...
    view_build_parser.add_argument("--json", action="store_true", help="emit machine-readable JSON")
    view_build_parser.add_argument(
        "--check",
        action="store_true",
        help="check whether existing generated markdown is current",
    )
    view_build_parser.set_defaults(func=cmd_view_build)
