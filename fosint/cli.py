...
def changed_view_targets(root: Path, records: list[Record], base_ref: str) -> list[Record]:
    changes, error = changed_git_paths(root, base_ref)
    if error:
        raise ValueError(f"failed to diff `{base_ref}`: {error}")
    ...
    for change in changes:
        ...
        try:
            ...
            raise ValueError(f"{change['path']}: failed to load YAML: {exc}") from exc
        if loaded.get("kind") in VIEW_TARGET_KINDS and isinstance(loaded.get("id"), str):
            target_ids.add(str(loaded["id"]))
    return [by_id[record_id] for record_id in sorted(target_ids) if record_id in by_id]
...
def render_pr_review_markdown(
    ...
    if chain_pages:
        lines.extend(
            [
                "| Target | Kind | Review State | Evidence | Sources | Open Questions | Open Challenges |",
                "| --- | --- | --- | ---: | ---: | ---: | ---: |",
            ]
        )
        ...
        lines.append("No changed current thesis or relationship records were found.")
    lines.append("")
    return "\n".join(lines)
